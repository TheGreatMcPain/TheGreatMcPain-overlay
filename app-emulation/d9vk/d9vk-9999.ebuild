...
src_prepare() {
	...
	cp "setup_dxvk.sh" "d9vk-setup"
	sed \
		-e "s#with_dxgi=1#with_dxgi=0#" \
		-e "s#\$action d3d1.*##" \
		-e "s#basedir=.*#basedir=\"${EPREFIX}/usr\"#" -i "d9vk-setup" || die

	bootstrap_dxvk() {
		# Set DXVK location for each ABI
		sed -e "s#x$(bits)#$(get_libdir)/d9vk#" -i "${S}/d9vk-setup" || die
...
multilib_src_configure() {
	local emesonargs=(
		...
		--libdir="$(get_libdir)/d9vk"
		--bindir="$(get_libdir)/d9vk/bin"
		-Denable_tests=false
		-Denable_dxgi=false
		-Denable_d3d10=false
		-Denable_d3d11=false
